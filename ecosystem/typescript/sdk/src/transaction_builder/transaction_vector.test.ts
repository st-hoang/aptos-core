...
describe('Transaction builder vector test', () => {
  it('should pass on script function payload', () => {
    ...
    vector.forEach(({ raw_txn, signed_txn_bcs, private_key }) => {
      ...
      doVerification(raw_txn, scriptFunctionPayload, private_key, signed_txn_bcs);
    });
  });
  ...
  it('should pass on script payload', () => {
    ...
    vector.forEach(({ raw_txn, signed_txn_bcs, private_key }) => {
      ...
      doVerification(raw_txn, scriptPayload, private_key, signed_txn_bcs);
    });
  });
  ...
  it('should pass on module payload', () => {
    ...
    vector.forEach(({ raw_txn, signed_txn_bcs, private_key }) => {
      ...
      doVerification(raw_txn, modulePayload, private_key, signed_txn_bcs);
    });
  });
});
