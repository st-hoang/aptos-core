...
module AptosFramework::SimpleMap {
    ...
    public fun count<Key: store, Value: store>(map: &SimpleMap<Key, Value>): u64 {
    ...
    public fun add_remove_many() {
        ...
        assert!(count(&map) == 0, 0);
        assert!(!contains_key(&map, &3), 1);
        insert(&mut map, 3, 1);
        assert!(count(&map) == 1, 2);
