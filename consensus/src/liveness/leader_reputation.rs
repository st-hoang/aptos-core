...
impl AptosDBBackend {
    ...
    fn refresh_window(
        ...
        for event in events {
            ...
            if known_round > 0
                && known_round == e.round()
                && known_version != event.transaction_version
            {
                error!(
                    "Agreed upon history we rely on proposer election has changed, for round {} from version {} to {}",
                    known_round,
                    known_version,
...
impl MetadataBackend for AptosDBBackend {
    fn get_block_metadata(&self, target_round: Round) -> Vec<NewBlockEvent> {
        ...
        {
            if let Err(e) = self.refresh_window(target_round, known_version, known_round) {
                error!(
